...
use crate::{
    ...
    polynomial,
    Commitment, FriedaError, Result, M31, SampleResult
};
...
pub fn verify_sampling(commitment: &Commitment, result: &SampleResult) -> Result<bool> {
    if result.indices.len() != result.values.len() {
        return Err(FriedaError::InvalidInput("Indices and values must have the same length".to_string()));
    }
    ...
    if result.indices.len() < samples_needed {
        return Err(FriedaError::InvalidInput(
            format!("Not enough samples: got {}, need {}", result.indices.len(), samples_needed)
        ));
    }
...
pub fn aggregate_sampling(results: &[SampleResult]) -> Result<SampleResult> {
    if results.is_empty() {
        return Err(FriedaError::InvalidInput("No results to aggregate".to_string()));
    }
...
mod tests {
    ...
    fn test_calculate_samples_needed() {
        ...
        let samples_higher_security = calculate_samples_needed(domain_size, degree, security_param * 2);
